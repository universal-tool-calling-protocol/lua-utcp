...
function Client.new(cfg)
  cfg=cfg or {}; local self=setmetatable({config=cfg,registry=Registry.new(),providers={}},Client)
...
function Client:add_provider(p)
  ...
  if p.manual then self:add_manual(p.manual,p) elseif p.tools then self.registry:add_manual({tools=p.tools},p) end
  return p
end
...
function Client:add_manual(manual,provider)
  assert(manual and manual.tools,'UTCP manual must contain tools'); self.registry:add_manual(manual,provider)
end
...
function Client:_discover_provider(p)
  if p.manual then self:add_manual(p.manual,p); return p.manual end
  ...
  elseif p.provider_type=='mcp' or p.call_template_type=='mcp' then
    ...
    manual={manual_version='1.0',utcp_version='1.0',tools=tools}; self:add_manual(manual,p); return manual
  end
  if manual then self:add_manual(manual,p); return manual end
...
function Client:call_tool(name,args)
  ...
  local transport=transports[typ].new(cfg)
...
function Client:call_tool_stream(name,args,on_event)
  ...
  if typ=='sse' then return transports.sse.new(cfg):listen(tpl.url or cfg.url,on_event)
  elseif typ=='streamable' then return transports.streamable.new(cfg):call(tpl,args or {},on_event)
