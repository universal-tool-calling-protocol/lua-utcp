...
--
-- The unified diff is generated by the canonical diff.unified tool.
-- The patch is applied by the canonical filesystem.patch tool.
...
--
-- Some transports may return a JSON encoded result.
-- Normalize it only if necessary.
--
...
if type(diff_result) ~= 'string' then
  ...
  assert(
    type(decoded) == 'string',
    'diff.unified did not return a patch string'
  )
...
assert(
  diff_result ~= '',
  'diff.unified returned an empty patch'
)
...
local patch_result, patch_error =
  client:call_tool(
    'filesystem.patch',
    {
      patch = diff_result,
...
if patch_result == nil then
  print(
    '--- filesystem.patch error ---'
  )
